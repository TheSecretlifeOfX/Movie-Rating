// Utility to check if the URL is valid
const isValidUrl = (url: string) => {
  try {
    new URL(url); // Try constructing a URL object
    return true;
  } catch (e) {
    return false;
  }
      ...
      try {
        ...
        if (data.Response === "False") {
          ...
          const formattedMovies: Movie[] = data.Search.map((item: Movie) => ({
            ...
            title: item.Title,
            year: item.Year,
            // Ensure valid URL for poster
            poster: isValidUrl(item.Poster) ? item.Poster : "https://via.placeholder.com/300x400.png",
          }));
