...
export default function Page() {
  ...
  // Fetch movies based on the page number
  useEffect(() => {
    async function fetchMovies() {
      setLoading(true); // Set loading state to true when fetching
      const url = `https://moviesdatabase.p.rapidapi.com/titles?page=${page}&limit=10`; // Adjust limit if needed
      const options = {
        method: "GET",
        headers: {
      ...
      try {
        ...
        if (data.results) {
          ...
          // Update the total number of pages (this should come from the API response)
    ...
    fetchMovies();
  }, [page]); // Run the effect whenever the page changes
