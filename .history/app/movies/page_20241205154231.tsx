...
const MoviesListPage = () => {
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (data.Response === "False") {
          ...
          const formattedMovies: Movie[] = data.Search.map((item: Movie) => ({
            ...
            title: item.Title,
            year: item.Year,
            // If Poster is "N/A", fallback to a placeholder image
    ...
    fetchMovies();
  }, [currentPage, searchQuery]); // Ensure fetchMovies is run when these values change

  const handleNextPage = () => {
    setCurrentPage(prevPage => prevPage + 1); // Using the functional update to prevent async issues
  };
