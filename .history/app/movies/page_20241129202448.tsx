    ...
    async function fetchMovies() {
      try {
        const response = await fetch(
          `http://www.omdbapi.com/?apikey=${API_KEY}` // Example query to fetch a list of movies
        );
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {movies.map((movie) => (
            <div key={movie.imdbID} className="bg-white shadow-md rounded-lg p-4">
              {/* Movie Poster */}
              <img
                src={movie.Poster !== "N/A" ? movie.Poster : "http://img.omdbapi.com/?apikey=${API_KEY}"} // Use a placeholder image if Poster is "N/A"
                alt={movie.Title}
